...
def connection_parameters() -> dict:
    """Read Snowflake credentials only when a connection is requested."""
    try:
        ...
        st.error(
            "Credenciais nao encontradas. Crie .streamlit/secrets.toml "
            "a partir de .streamlit/secrets.toml.example."
        )
        st.stop()
...
def open_read_only_session() -> Session:
    ...
    parameters["role"] = st.secrets["snowflake"].get(
        "query_role", "COVID_READONLY_ROLE"
    )
...
def prepare_downloaded_data() -> pd.DataFrame:
    ...
    df = pd.read_csv(source, usecols=REQUIRED_COLUMNS, low_memory=False)
...
def normalize_snowflake_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in NUMERIC_COLUMNS:
        df[column] = pd.to_numeric(df[column], errors="coerce")
    return df.dropna(subset=["location", "date"]).sort_values(
        ["location", "date"]
    )
...
def load_into_snowflake() -> None:
    ...
    try:
        ...
        st.sidebar.success(
            f"{len(df):,} registros gravados em {TABLE_NAME}.".replace(",", ".")
        )
...
def load_from_snowflake() -> None:
    ...
    try:
        with st.spinner("Consultando o Snowflake..."):
            ...
            config = connection_parameters()
            full_name = (
                f'{config["database"]}.{config["schema"]}.{TABLE_NAME}'
            )
        ...
        st.sidebar.error(
            "Nao foi possivel carregar a tabela. Execute primeiro a carga de dados. "
            f"Detalhes: {error}"
        )
    finally:
        if session is not None:
...
st.caption("Dados da Our World in Data armazenados e consultados no Snowflake")

st.sidebar.header("Snowflake")
st.sidebar.caption(f"Tabela: TEST_DB.PUBLIC.{TABLE_NAME}")

if st.sidebar.button(
    "Carregar dados no Snowflake", type="primary", use_container_width=True
):
    load_into_snowflake()

if st.sidebar.button("Carregar dashboard", use_container_width=True):
    load_from_snowflake()

if "covid_data" not in st.session_state:
    st.info(
        "Use os botoes da barra lateral: primeiro carregue os dados no Snowflake "
        "e depois carregue o dashboard."
    )
    st.stop()

data = st.session_state["covid_data"]
available_countries = sorted(data["location"].dropna().unique().tolist())
...
if not isinstance(selected_period, (tuple, list)) or len(selected_period) != 2:
    st.warning("Selecione as datas inicial e final do periodo.")
    st.stop()
...
if filtered.empty:
    st.warning("Nao existem dados para os filtros selecionados.")
    st.stop()

latest = (
    filtered.sort_values("date")
    .groupby("location", as_index=False, group_keys=False)
    .tail(1)
    .copy()
)
...
kpi_cases.metric("Total de casos", format_integer(total_cases))
kpi_deaths.metric("Total de obitos", format_integer(total_deaths))
kpi_countries.metric("Paises analisados", latest["location"].nunique())
kpi_latest.metric("Data mais recente", latest["date"].max().strftime("%d/%m/%Y"))
...
with tab_cases:
    st.subheader("Evolucao de casos novos por pais")
    cases_chart = px.line(
        filtered,
        x="date",
        y="new_cases",
    ...
    cases_chart.update_layout(legend_title_text="Pais")
    st.plotly_chart(cases_chart, use_container_width=True)

with tab_deaths:
    st.subheader("Total de obitos por pais")
    deaths_chart = px.bar(
        latest.sort_values("total_deaths", ascending=False),
        x="location",
        y="total_deaths",
        color="location",
        text_auto=".3s",
        labels={"location": "Pais", "total_deaths": "Total de obitos"},
    )
    deaths_chart.update_layout(showlegend=False)
    st.plotly_chart(deaths_chart, use_container_width=True)

with tab_vaccines:
    st.subheader("Proporcao da populacao vacinada com ao menos uma dose")
    vaccination = latest.copy()
    vaccination["vaccinated_pct"] = (
        vaccination["people_vaccinated"] / vaccination["population"] * 100
    ).clip(lower=0, upper=100)
    vaccine_chart = px.bar(
        vaccination.sort_values("vaccinated_pct", ascending=False),
        x="location",
        y="vaccinated_pct",
        color="location",
        text_auto=".1f",
        range_y=[0, 100],
        labels={"location": "Pais", "vaccinated_pct": "Populacao vacinada (%)"},
    )
    vaccine_chart.update_layout(showlegend=False)
    st.plotly_chart(vaccine_chart, use_container_width=True)

with tab_scatter:
    st.subheader("Relacao entre populacao e total de casos")
...
with tab_raw:
    ...
    display_data = filtered.sort_values(["date", "location"], ascending=[False, True])
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name=(
            f"covid_owid_{date.today().isoformat()}_filtrado.csv"
        ),
        mime="text/csv",
    )
...
with tab_sql:
    ...
    st.caption("Por seguranca, apenas uma instrucao SELECT ou WITH e permitida.")
    ...
    if st.button("Executar consulta SQL"):
        if not is_read_only_query(sql_query):
            st.error("Digite uma unica consulta iniciada por SELECT ou WITH.")
        else:
            query_session = None
            try:
                with st.spinner("Executando consulta no Snowflake..."):
                    query_session = open_read_only_session()
                    query_result = query_session.sql(sql_query).to_pandas()
                st.success(f"Consulta concluida: {len(query_result)} linha(s).")
